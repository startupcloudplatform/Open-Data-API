...
class sub_requestList extends React.Component 
{
  constructor(...props) {
    super(...props);
    ...
    this.state.request_title = this.props.request_title;
  ...
  handleCardClick(request_no)
  {
    this.props.returnType(request_no);
  }
  ...
  _getData = async () => {
    ...
    var requesturl = config.serverpoint + "/request"
    ...
    .then( response => 
    {
        ...
        for(var i = 0 ; i < response.data.length ; i++)
        {
            ...
            tempdatapack.request_no = response.data[i].request_no;    
            tempdatapack.request_title = response.data[i].request_title;    
            ...
            tempdatapack.admin_reply = response.data[i].admin_reply;
    ...
    var requesturls = config.serverpoint + "/request/target/list"
    ...
    if(sessionStorage.getItem("user") == 'admin'){
      var requesturl = config.serverpoint + "/request/admin"
  ...
  render()
  {
    const { rows, rows2,rows3, rowsPerPage, page } = this.state;
          ...
          {this.state.fetching2 == false && rows2
              ...
              .map(row => 
              {
                return (    
                     ...
                     <tr onClick={(e) => this.handleCardClick(row.request_no)}>
...
export default sub_requestList;
