...
router.post('/', function(req, res, next) {
    ...
    let transporter = nodemailer.createTransport({
        service: 'gmail',
        auth: {
            user: 'admin_mail@gmail.com',  // gmail 계정 아이디를 입력
            pass: 'password'          // gmail 계정의 비밀번호를 입력
        }
    });

    let mailOptions = {
        from: 'admin_mail@gmail.com',   // 발송 메일 주소 (위에서 작성한 gmail 계정 아이디)
        to: 'admin_mail@gmail.com' ,                     // 수신 메일 주소
    ...
    transporter.sendMail(mailOptions, function(error, info){
        if (error) {
            ...
            var sql = mysql.format(
                'INSERT INTO odag.mail (user_name, email, mail_title, mail_contents) ' +
                'values (?,?,?,?)',
...
router.post('/api_cancle', function(req, res, next) {
    ...
    transporter.sendMail(mailOptions, function(error, info){
        if (error) {
            ...
            var sql = mysql.format(
                'INSERT INTO odag.mail_admin_cancle (email, mail_title, mail_contents, user_name, provider) ' +
                'values (?,?,?,?,?)',
                ...
                conn.query(sql, function(err, rows) {
                    if(err) { throw err; }
                        ...
                        var sql2 = mysql.format(
                            'update odag.openapi set odag.openapi.progress = \'서비스 중지\' where openapi_no = ?',
                            [openapi_no]
                        );
                        pool.getConnection(function (err, conn) {
            ...
            pool.getConnection(function(err, conn){
                conn.query(sql, function(err, rows) {
                    if(err) { throw err; }
                        ...
                        var sql2 = mysql.format(
                            'update odag.filedata set odag.filedata.progress = \'서비스 중지\' where filedata_id = ?',
                            [filedata_id]
                        );
                        pool.getConnection(function (err, conn) {
                            conn.query(sql2, function(err, rows2){
