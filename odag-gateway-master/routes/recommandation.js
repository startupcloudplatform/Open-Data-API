...
router.get('/popular/api', function(req, res, next) {
    var sql = mysql.format(
        'select a.openapi_no, a.name, a.provider, a.comment, date_format(a.update_time, \'%Y-%m-%d\') update_time, a.views_count, b.category, a.activity_type, a.views_count ,a.openapi_cnt_use,  type from odag.openapi a, odag.category_odag b where a.category = b.category_no order by openapi_cnt_use desc limit 10 ;'
    );
    console.log(sql);
...
router.get('/search', function(req, res, next) {
    var sql = mysql.format(
         // 'select @rownum:=@rownum+1 as ranking, x.* from (select search_keywords from odag.search group by search_keywords order by sum(search_count) desc limit 5) x, (select @rownum:=0) r'
    'select search_keywords from odag.search group by search_keywords order by sum(search_count) desc limit 5'
