...
router.get('/', function(req, res, next) {
    pool.getConnection(function(err, conn){
        conn.query("select user_name from odag.user", function(err, rows) {
            if(err) { throw err; }
...
router.post('/request/:openapi_no', function(req, res, next) {
    ...
    var sql = mysql.format(
        'SELECT * FROM odag.openapi WHERE odag.openapi.openapi_no = ?',
    ...
    pool.getConnection(function(err, conn){
        conn.query(sql, function(err, rows) {
            ...
            var sqlQuery = 'select * from odag.developer where odag.developer.user_id = \'' + id + '\'';
            ...
            pool.getConnection(function(err, conn){
                conn.query(sqlQuery, function(err, rows2) {
                    ...
                    if(rows2.length === 1){
                        ...
                        if(rows2[0].auth_key) {
                            var sqlQuery2 = 'select * from odag.service_matching where odag.service_matching.key= \'' + rows2[0].auth_key + '\' and odag.service_matching.service_id = \'' + openapi_no + '\'';
                                ...
                                conn.query(sqlQuery2, function(err,rows3){
                                    ...
                                    if(rows3.length === 1){
                                        ...
                                        var sqlQuery3 = mysql.format(
                                            'insert into odag.history(openapi_no , user_id , history_id, status , comment)' +
                                            'values (?,?,?,?,?)',
                                            ...
                                            conn.query(sqlQuery3, function (err,rows4) {
                                                ...
                                                if(rows4.length==1){
                                                    ...
                                                    var sql4 = mysql.format(
                                                        'update odag.history AS a, odag.openapi AS b, odag.provider AS c set a.provider = b.provider, a.provider_id = c.user_id where a.openapi_no = b.openapi_no and b.provider = c.provider_name'
                                                    );
                                                    pool.getConnection(function (err,conn) {
                                        ...
                                        var sqlQuery4 = mysql.format(
                                            'insert into odag.history(openapi_no , user_id , object, history_id, status , comment, provider)' +
                                                'values (?,?,?,?,?,?,?)',
                                            [req.params.openapi_no, req.body.user_id, req.body.object , '1', 'success', '활용 신청 완료 성공',provider]
                                        );
                                            ...
                                            conn.query(sqlQuery4, function (err,rows5) {
                                                ...
                                                if(rows5.length==1){
                                                    ...
                                                    var sql5 = mysql.format(
                                                        'update odag.openapi set openapi_cnt_use = openapi_cnt_use + 1 where openapi_no = \'' +  openapi_no2 + '\''
                                                    );
                                                    pool.getConnection(function (err,conn) {
                                                    ...
                                                    var sql6 = mysql.format(
                                                        'update odag.history a, odag.provider b set a.provider_id = b.user_id where a.provider = b.provider_name'
                                                    )
                                                    pool.getConnection(function (err,conn) {
...
router.delete('/request/:openapi_no/:user_id', function(req, res, next) {
    ...
    var sql = mysql.format(
        'SELECT * FROM odag.openapi WHERE odag.openapi.openapi_no in (?)',
        [req.params.openapi_no]
    );
            ...
            pool.getConnection(function(err, conn){
                conn.query(sqlQuery, function(err, rows2) {
                    ...
                    if(rows2.length === 1){
                        ...
                        if(rows2[0].auth_key) {
                            var sqlQuery2 = 'select * from odag.service_matching where odag.service_matching.key= \'' + rows2[0].auth_key + '\' and odag.service_matching.service_id in (\'' + openapi_no + '\')';
                            ...
                            pool.getConnection(function (err,conn) {
                                conn.query(sqlQuery2, function(err,rows3){
                                    if(err) { throw err;}
                                    if(rows3.length !== 0){
                                        var sqlQuery3 = 'delete from odag.service_matching where odag.service_matching.key= \'' + rows2[0].auth_key + '\' and odag.service_matching.service_id in (\'' + openapi_no + '\')';
