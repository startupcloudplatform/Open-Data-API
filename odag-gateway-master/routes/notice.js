...
router.post('/', function(req, res, next) {
    ...
    var sql = mysql.format(
        'insert into odag.notice (notice_title, notice_contents, create_user_id) ' +
        'values (?,?,\'운영자\')',
...
router.put('/:notice_id', function(req, res, next) {
    ...
    var sql = mysql.format(
        'UPDATE odag.notice SET notice_title = ?, notice_contents = ?, update_user_no = ?, update_time = CURRENT_TIMESTAMP where notice_no = ?',
            ...
            if (rows.length === 0) {
                ...
                for (var a=0;a<rows.length;a++) {
                    ...
                    var sql2 = mysql.format(
                        'update odag.notice set odag.notice.count = odag.notice.count + 1 where notice_no = \'' + notice_no2 + '\''
                    );
...
router.put('/targetup/:notice_no', function(req, res, next) {
    ...
    var sql = mysql.format(
        'UPDATE odag.notice SET target = 1 where notice_no = ? ',
        [req.params.notice_no]
    );
...
router.put('/targetdown/:notice_no', function(req, res, next) {
    ...
    var sql = mysql.format(
        'UPDATE odag.notice SET target = 0 where notice_no = ? ',
        [req.params.notice_no]
    );
...
router.put('/notice_no/:notice_no/comment', function(req, res, next) {
    ...
    var sql = mysql.format(
        'UPDATE odag.notice SET notice_title = ?, notice_contents = ?, update_time = CURRENT_TIMESTAMP where notice_no = ?',
