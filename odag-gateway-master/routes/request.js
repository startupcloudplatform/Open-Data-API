            ...
            if (rows.length === 0) {
                ...
                for (var a=0;a<rows.length;a++) {
                    ...
                    var sql2 = mysql.format(
                        'update odag.request set odag.request.count = odag.request.count + 1 where request_no = \'' + request_no2 + '\''
                    );
...
router.post('/', function(req, res, next) {
    ...
    var sql = mysql.format(
        'INSERT INTO odag.request (request_title, request_contents, user_id) ' +
        'values (?,?,?)',
...
router.put('/:request_no', function(req, res, next) {
    ...
    var sql = mysql.format(
        'UPDATE odag.request SET request_title = ?, request_contents = ?, user_id = ?, update_time = CURRENT_TIMESTAMP where request_no = ?',
...
router.put('/targetup/:request_no', function(req, res, next) {
    ...
    var sql = mysql.format(
        'UPDATE odag.request SET target = 1 where request_no = ? ',
        [req.params.request_no]
    );
...
router.put('/targetdown/:request_no', function(req, res, next) {
    ...
    var sql = mysql.format(
        'UPDATE odag.request SET target = 0 where request_no = ? ',
        [req.params.request_no]
    );
...
router.post('/request_no/:request_no/comment', function(req, res, next) {
    ...
    var sql = mysql.format(
        'INSERT INTO odag.request_reply (request_no, request_reply_contents, user_id) ' +
        'values (?,?,?)',
    ...
    pool.getConnection(function(err, conn){
        conn.query(sql, function(err, rows) {
            ...
            if(req.body.user_id == 'admin'){
                var request_no2 = req.params.request_no;
                var sql2 = mysql.format(
                  'update odag.request set odag.request.admin_reply = \'답변완료\' where odag.request.request_no = \'' + request_no2 + '\''
                );
...
router.put('/:request_no/comment/:comment_id', function(req, res, next) {
    ...
    var sql = mysql.format(
        'UPDATE odag.request_reply SET request_reply_contents = ?, update_time = CURRENT_TIMESTAMP where request_no = ? and request_reply_no = ?',
