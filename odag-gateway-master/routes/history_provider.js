...
router.get('/api/date/:openapi_no/success',function (req,res,next) {
    var sql = mysql.format(
        ...
        'AND odag.history.comment = "openapi 이용 성공"\n' +
        'AND odag.history.update_time < NOW()\n' +
        'AND odag.history.update_time > DATE_SUB(NOW(), INTERVAL 1 MONTH)' +
...
router.get('/api/hour/:openapi_no/success',function (req,res,next) {
    var sql = mysql.format(
        ...
        'AND odag.history.comment = "openapi 이용 성공"\n' +
        'AND odag.history.update_time < NOW()\n' +
        'AND odag.history.update_time > DATE_SUB(NOW(), INTERVAL 1 DAY)' +
...
router.get('/filedata/date/:filedata_no/success',function (req,res,next) {
    var sql = mysql.format(
        'SELECT DATE_FORMAT(`update_time`, \'%Y-%m-%d\') date, count(*) cnt ' +
        'FROM odag.history_filedata ' +
        'WHERE filedata_no = ? and status = \'success\'  ' +
        'AND odag.history_filedata.comment = "다운로드 성공"\n' +
        'AND odag.history_filedata.update_time < NOW()\n' +
        'AND odag.history_filedata.update_time > DATE_SUB(NOW(), INTERVAL 1 MONTH)' +
        'GROUP BY DATE_FORMAT(`update_time`, \'%Y-%m-%d\') ORDER BY date;',
...
router.get('/filedata/hour/:filedata_no/success',function (req,res,next) {
    var sql = mysql.format(
        ...
        'AND odag.history_filedata.comment = "다운로드 성공"\n' +
        'AND odag.history_filedata.update_time < NOW()\n' +
        'AND odag.history_filedata.update_time > DATE_SUB(NOW(), INTERVAL 1 DAY)' +
        'GROUP BY DATE_FORMAT(`update_time`, \'%Y-%m-%d %H시\') ORDER BY date;',
        [req.params.filedata_no]
    );
    console.log(sql);
...
router.get('/api/date/:provider_id/success/use',function (req,res,next) {
    var sql = mysql.format(
        ...
        'FROM odag.history ' +
        'WHERE provider_id = ? and status = \'success\'  ' +
        'AND odag.history.comment = "활용 신청 완료 성공"\n' +
        'AND odag.history.update_time < NOW()\n' +
        'AND odag.history.update_time > DATE_SUB(NOW(), INTERVAL 1 MONTH)' +
        'GROUP BY DATE_FORMAT(`update_time`, \'%Y-%m-%d\') ORDER BY date;',
        [req.params.provider_id]
...
router.get('/filedata/date/:provider_id/all',function (req,res,next) {
    var sql = mysql.format(
        ...
        'FROM odag.history_filedata ' +
        'WHERE provider_id = ? ' +
        'AND odag.history_filedata.comment = "조회 성공"\n' +
        'AND odag.history_filedata.update_time < NOW()\n' +
        'AND odag.history_filedata.update_time > DATE_SUB(NOW(), INTERVAL 1 MONTH)' +
        'GROUP BY DATE_FORMAT(`update_time`, \'%Y-%m-%d\') ORDER BY date;',
        [req.params.provider_id]
...
router.get('/provider/:provider_id', function(req, res, next) {
    ...
    var sql = mysql.format(
        'select comment from odag.history_filedata where provider_id = ? and comment = \'조회 성공\'',
        [req.params.provider_id]
    );
...
router.get('/provider/:provider_id/download', function(req, res, next) {
    ...
    var sql = mysql.format(
        'select comment from odag.history_filedata where provider_id = ? and comment = \'다운로드 성공\'',
        [req.params.provider_id]
    );
...
router.get('/provider_download/:filedata_id/download', function(req, res, next) {
    ...
    var sql = mysql.format(
        'select count(*) cnt from odag.history_filedata where filedata_no = ? and comment = \'다운로드 성공\'',
...
router.get('/provider/:provider_id/success', function(req, res, next) {
    ...
    var sql = mysql.format(
        'select count(*) cnt from odag.history where provider_id = ? and comment = \'활용 신청 완료 성공\'',
        [req.params.provider_id]
    );
...
router.get('/provider/:provider_id/use', function(req, res, next) {
    ...
    var sql = mysql.format(
        'select count(*) cnt from odag.history where provider_id = ? and comment = \'openapi 이용 성공\'',
        [req.params.provider_id]
    );
...
router.get('/provider/:provider_id/fail', function(req, res, next) {
    ...
    var sql = mysql.format(
        'select count(*) cnt from odag.history where provider_id = ? and status = \'fail\'',
        [req.params.provider_id]
    );
...
router.get('/provider_api/:user_id', function(req, res, next) {
    ...
    var sql = mysql.format(
        'select date_format(a.create_time, \'%Y-%m-%d\')creates_time, a.*, b.cnt, d.cnt_file from odag.provider a, odag.openapi c, (select count(*) cnt from odag.openapi, odag.provider where openapi.provider = provider.provider_name and provider.user_id = ? and openapi.progress = \'등록 완료\') b, (select count(*) cnt_file from odag.filedata, odag.provider where filedata.provider = provider.provider_name and provider.user_id = ? and filedata.progress = \'등록 완료\') d where a.user_id = ? group by a.user_id',
