...
router.get('/:user_id', function(req, res, next) {
    var sql = mysql.format(
        'select odag.bookmark.bookmark_no, odag.bookmark.bookmark_name, odag.bookmark.user_id, odag.developer.user_id from odag.bookmark, odag.developer where odag.bookmark.user_id = odag.developer.user_id and odag.developer.user_id = ?',
        [req.params.user_id]
    );
...
router.post('/:user_id', function(req, res, next) {
    ...
    var sql = mysql.format(
        'select user_id from odag.developer where odag.developer.user_id = ?',
        [req.params.user_id]
    );
        ...
        pool.getConnection(function(err, conn){
            conn.query(sql, function(err, rows) {
                ...
                var sql2 = mysql.format(
                    'insert into odag.bookmark (bookmark_name, user_id) ' +
                    'values (?,?)',
...
router.put('/:bookmark_no', function(req, res, next) {
    ...
    var sql = mysql.format(
        'UPDATE odag.bookmark SET bookmark_name = ? where bookmark_no = ?',
...
router.post('/:user_id/:bookmark_no', function(req, res, next) {
    ...
    // var sql = mysql.format(
    //     'insert into odag.bookmark_openapi (bookmark_no, openapi_no) select ?, ? from DUAL where not exists (select * from bookmark_openapi where openapi_no = ?)',
    //     [req.params.bookmark_no, req.body.openapi_no, req.body.openapi_no]
    // );
    var sql = mysql.format(
        'insert into odag.bookmark_openapi (bookmark_no, openapi_no) select ?, ? where not exists (select * from bookmark_openapi where bookmark_no = ? and openapi_no = ?)',
...
router.post('/download', function(req, res, next) {
    ...
    var sql = mysql.format(
        'insert into odag.bookmark_openapi (bookmark_no, openapi_no) ' +
        'values (?,?)',
