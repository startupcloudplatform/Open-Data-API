    ...
    if(streams2 != undefined && streams2.length > 0){
        for(i=0;i<streams2.length; i++){
            ...
            var sql = mysql.format(
                'INSERT ignore INTO odag.openapi (openapi.name, openapi.category, openapi.provider, openapi.comment, openapi.openapi_join_url, openapi.keyword, openapi.key, openapi.parameters, openapi.type) values (?)',
                [streams2]
            );
        }
    }
...
router.post('/file/csv', function(req, res, next) {
    ...
    for(i=1;i<streams2.length-1; i++){
        ...
        var sql = mysql.format(
            'INSERT ignore INTO odag.filedata (filedata.name^ filedata.category^ filedata.provider^ filedata.comment^ filedata.filedata_use_url^ filedata.keyword^ filedata.type) values ?',
            [values],
        );
    }
    ...
    for(i=1;i<streams2.length-1; i++){
        ...
        var sql2 = mysql.format(
            'INSERT ignore INTO odag.filedata_detail (filedata_detail.filedata_use_url^ filedata_detail.category^ filedata_detail.provider^ filedata_detail.name^ filedata_detail.filedata_key) values ?',
            [values3],
        );
