...
router.get('/data', function(req, res, next) {
    ...
    var sql = mysql.format(
        'select a.openapi_no, a.name, b.category, a.provider, a.comment, a.views_count, a.activity_type, a.type, date_format(a.update_time, \'%Y-%m-%d\')update_time from odag.openapi a, odag.category_odag b ' + i +' \n' +
        'union all \n' +
        'select a.filedata_id, a.name, b.category, a.provider, a.comment, a.views_count, a.activity_type, a.type,date_format(a.update_time, \'%Y-%m-%d\')update_time from odag.filedata a, odag.category_odag b ' + i +' \n' +
        'order by name',
        value
    );
        ...
        conn.query(sql, function(err, rows) {
            ...
            if (rows.length === 0) {
                ...
                var sql2 = mysql.format(
                    'update odag.search set user_id = ?, search_count = search_count + 1 ,search_time = CURRENT_TIMESTAMP where search_keywords = \'' + keyword + '\' and odag.search.user_id = \'' + userid + '\'',
                    // 'update odag.search set search_count = search_count + 1 ,search_time = CURRENT_TIMESTAMP where search_keyword = ? and odag.search.user_id = \'' + userid + '\'',
                        [req.body.user_id]
                );
                ...
                pool.getConnection(function (err,conn) {
                    conn.query(sql2, function (err, rows2) {
                        ...
                        if(rows2.length === 1){
                            ...
                            var sql3 = 'select * from odag.search where odag.search.search_keywords = \'' + keyword + '\' and odag.search.user_id = \'' + userid + '\'';
                                ...
                                conn.query(sql3, function (err,rows3) {
                                    ...
                                    if(rows3.length === 0){
                                        ...
                                        var sql4 = mysql.format(
                                            ...
                                            'insert into odag.search(search_keywords, user_id, search_time) '+
    ...
    pool.getConnection(function(err, conn){
        conn.query(sql, function(err, rows) {
            ...
            if (rows.length === 0) {
                ...
                var sql2 = mysql.format(
                    'update odag.search set search_count = search_count + 1 ,search_time = CURRENT_TIMESTAMP where search_keywords = \'' + keyword + '\' and odag.search.user_id = \'' + userid + '\'',
                    [req.query.name]
                );
...
router.get('/tool', function(req, res, next) {
    ...
    var sql = mysql.format(
        'select a.tool_no, a.name, b.category, a.provider, a.comment, a.views_count, a.activity_type, a.type,date_format(a.update_time, \'%Y-%m-%d\')update_time from odag.tool a, odag.category_tool b ' + i +' \n' +
        'order by name',
        value
    );
...
router.get('/filedata', function(req, res, next) {
    ...
    //
    // var sql = mysql.format(
    //     'select a.filedata_id, a.name, b.category, a.provider, a.comment, a.views_count, a.activity_type, a.type, a.update_time from odag.filedata a, odag.category_odag b ' + i +' \n' +
    //     'order by name',
    //     value
    // );
...
var options = { method: 'GET',
  url: 'http://127.0.0.1:9000/apiservice/`+ serviceid +`',
  qs: { 
     auth_key: '` + key + `',
  ` + qs +  `}
};
